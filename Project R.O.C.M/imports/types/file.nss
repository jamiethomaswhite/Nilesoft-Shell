            ...
            //WINDOWS INTERNAL FUNCTIONS
                menu(title='Windows Internal Context Menu Entries' image=\uE25F /*https://nilesoft.org/glyphs/e25f*/ tip='A placeholder for the Windows internal context menu entries' type='file' expanded='true' position='middle')
                {
                    ...
                    //OPEN ORIGINAL LOCATION
                        modify(title='Open original location' image=\uE0D5 /*https://nilesoft.org/glyphs/e0d5*/ tip='Open the true location of the currently selected item/s' type='file' where=str.equals(this.name, ["open file location", "open folder location"]) separator='both' position='middle')
                    //OPEN ORIGINAL LOCATION

                    //RUN WITH POWERSHELL
                        modify(title='Run with PowerShell' image=\uE218 /*https://nilesoft.org/glyphs/e218*/ tip='Run the currently selected item/s with PowerShell' type='file' where=str.equals(this.name, ["run with powershell"]) separator='after' position=indexof('open', 1))
                    //RUN WITH POWERSHELL

                    //RUN AS A DIFFERENT USER
                        modify(title='Run as a different user' image=\uE09F /*https://nilesoft.org/glyphs/e09f*/ tip='Run the currently selected item/s as another user' type='file' where=str.equals(this.name, ["run as a different user"]) separator='after' position=indexof('run as administrator', 0))
                    //RUN AS A DIFFERENT USER

                    //RUN AS ADMINISTRATOR
                        modify(title='Run as administrator' image=\uE100 /*https://nilesoft.org/glyphs/e100*/ tip='Run the currently selected item/s as an admin user' type='file' where=str.equals(this.name, ["run as administrator"]) separator='after' position=indexof('open', 1))
                    //RUN AS ADMINISTRATOR

                    //CUT
                        modify(title='Cut' image=\uE0B8 /*https://nilesoft.org/glyphs/e0b8*/ tip='Cut the currently selected item/s and hold in the clipboard for destructive pasting' keys='Ctrl+X' type='file' where=str.equals(this.name, ["cut"]) separator='before' position=indexof('run as administrator', 1))
                    //CUT
